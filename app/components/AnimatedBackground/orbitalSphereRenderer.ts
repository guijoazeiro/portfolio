// Adapted from ThreeUI's MIT-licensed Orbital Sphere renderer.
// Source revision documented as SHA-256 2fde5ec9da60 by ThreeUI.
...
export function createOrbitalSphereRenderer(
  ...
  const colorBright = new THREE.Color(0x93a8ff);
  const colorDim = new THREE.Color(0x2445a6);
  let validIndex = 0;
  ...
  for (let index = 0; index < 6; index += 1) {
    ...
    if (index % 2 !== 0) {
      ...
      const nodeMaterial = new THREE.MeshBasicMaterial({ color: 0x93a8ff });
  ...
  return {
    resize(width: number, height: number) {
      ...
      if (width >= 1024) {
        networkGroup.position.set(2.8, -0.15, -2);
        responsiveScale = 1.18;
        camera.position.z = 5.5;
      } else {
        networkGroup.position.set(1.45, -0.25, -3);
    ...
    render() {
      ...
      networkGroup.children.forEach((child, index) => {
        if (child.type === "Line") {
          child.rotation.z +=
            0.0004 * options.speed * (index % 2 === 0 ? 1 : -1);
        }
      });
